...
def feature_func(traj):
    ...
    return np.array([max_pos, max_velocity_y, min_velocity_y])
...
def save_user_parameters_to_csv(order_type, parameters):
    ...
    file_name = f"high_five_user_parameters_{order_type}.csv"
...
def main():
    ...
    state_filename = f"data_order/high_five/states_{order_type}_rec.csv"
    action_filename = f"data_order/high_five/actions_{order_type}_rec.csv"
    ...
    robot_agent.say("Hi! Welcome to our experiments! This will be the experiment about a high five")
    sleep(1.0)
    ...
    # Save the last estimated user parameters to a CSV file
